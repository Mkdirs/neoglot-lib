...
#[test]
fn group_test() {
    let regex = ChrRegex::new().then(RegexElement::Group(
        vec![
            RegexElement::Item('a', Quantifier::OneOrMany),
...
#[test]
fn negation() {
    let regex = ChrRegex::new().then(RegexElement::NoneOf(
        vec![
            RegexElement::Set('a', 'z', Quantifier::Exactly(1)),
            RegexElement::Set('A', 'Z', Quantifier::Exactly(1)),
        ],
